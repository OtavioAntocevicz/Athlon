...
import { ArrowLeft, Megaphone, Clock, CheckCircle2 } from "lucide-react";
...
export function AvisosProfessorPage() {
  ...
  const enviar = useMutation({
    ...
    onSuccess: () => {
      setMensagem(agendar ? "Aviso agendado com sucesso!" : "Aviso enviado aos alunos!");
    ...
    <AppShell>
      ...
      <Card className="space-y-4 p-4">
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {mensagem && (
            <p className="text-sm text-center text-muted-foreground">{mensagem}</p>
          )}
      ...
      <h2 className="mt-8 mb-3 text-lg font-bold text-primary">Histórico de avisos</h2>
      ...
      <div className="space-y-3">
        ...
        {historico?.map((a) => (
          <Card key={a.id} className="p-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{a.descricao}</p>
