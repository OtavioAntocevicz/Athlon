...
export function EventosProfessorPage() {
  const navigate = useNavigate();
  const [filtro, setFiltro] = useState("todos");

  const { data, isLoading } = useQuery({
    queryKey: ["eventos", "professor"],
        ...
        <div className="mt-4 space-y-3">
          ...
          {!isLoading && filtrados.length === 0 && (
            <p className="py-8 text-center text-sm text-muted-foreground">
              {filtro === "todos"
                ? "Nenhum evento criado ainda. Crie amistosos e campeonatos no detalhe de cada turma."
                : filtro === "proximos"
          ...
          {filtrados.map((evento) => {
            ...
            return (
              <Card
                key={evento.id}
                className={cn(
                  "cursor-pointer border-primary/20 bg-card p-4 transition-transform active:scale-[0.99]",
                  evento.passado && "opacity-70",
                )}
                onClick={() => navigate(`/turmas/${evento.turmaId}`)}
              >
