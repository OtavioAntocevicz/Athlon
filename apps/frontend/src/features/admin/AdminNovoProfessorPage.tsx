...
export function AdminNovoProfessorPage() {
  ...
  return (
    <AdminShell>
      ...
      <PageHeader title="Novo professor" subtitle="O treinador usará estas credenciais para entrar no app" />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 max-w-lg space-y-4">
        ...
        <div>
          ...
          {errors.senha && <p className="mt-1 text-sm text-destructive">{errors.senha.message}</p>}
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium">
