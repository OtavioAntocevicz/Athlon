...
- `api.athlonsport.app.br` → Railway (CNAME verificado, certificado válido, `/health` ok)

Registros no Registro.br (já propagados):
...
O proxy da Vercel (`/api/v1` e `/api/cron`) aponta para `https://api.athlonsport.app.br`.
